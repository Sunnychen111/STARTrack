...
def get_center3x3(locations, centers, strides, window_range=3):
    '''
    Inputs:
        locations: M x 2
        centers: N x 2
    ...
    return (dist_x <= strides_expanded[:, :, 0] * radius) & \
           (dist_y <= strides_expanded[:, :, 0] * radius)
