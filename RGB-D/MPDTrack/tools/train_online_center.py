...
def _yaml_defaults_from_dict(data):
    ...
    section_key_map = {
        ...
        "loss": {
            ...
            "use_center_aware_target": "use_center_aware_target",
            "center_lambda": "center_lambda",
            "high_iou_preserve_thr": "high_iou_preserve_thr",
            "high_iou_preserve_gain": "high_iou_preserve_gain",
            "center_better_margin": "center_better_margin",
...
def parse_args():
    ...
    # Center-aware target selection for preserving high-precision localization.
    # This is enabled by default but can be disabled by setting
    # loss.use_center_aware_target: false in YAML or by passing
    # --no-use-center-aware-target on CLI.
    parser.add_argument("--use-center-aware-target", action=argparse.BooleanOptionalAction,
                        default=bool(_default(yaml_defaults, "use_center_aware_target", True)))
    parser.add_argument("--center-lambda", type=float,
                        default=float(_default(yaml_defaults, "center_lambda", 0.10)))
    parser.add_argument("--high-iou-preserve-thr", type=float,
                        default=float(_default(yaml_defaults, "high_iou_preserve_thr", 0.75)))
    parser.add_argument("--high-iou-preserve-gain", type=float,
                        default=float(_default(yaml_defaults, "high_iou_preserve_gain", 0.15)))
    parser.add_argument("--center-better-margin", type=float,
                        default=float(_default(yaml_defaults, "center_better_margin", 0.03)))
...
def compute_topk_center_errors(topk_bboxes_xywh, gt_xywh):
    """Compute normalized center error for top-K candidate boxes.

    Args:
        topk_bboxes_xywh: Tensor [K, 4], xywh in real image coordinates.
        gt_xywh: Tensor/list [4], xywh in real image coordinates.

    Returns:
        Tensor [K]. Smaller is better. Distance is normalized by
        sqrt(gt_w * gt_h), so the scale is comparable across objects.
    """
    if topk_bboxes_xywh.dim() != 2 or topk_bboxes_xywh.size(-1) != 4:
        raise ValueError(f"topk_bboxes_xywh must be [K, 4], got {tuple(topk_bboxes_xywh.shape)}")
    ...
    gt_scale = torch.sqrt((gt[2] * gt[3]).clamp_min(1.0))
    norm_error = dist / gt_scale
    return norm_error.nan_to_num(1e3).clamp_min(0.0)
...
def choose_update(logits_t, topk_bboxes_xywh, topk_feats, target_idx, valid, epoch, args, prev_state):
    ...
    scheduled_safe = (
        ...
        and candidate_motion_ok(topk_bboxes_xywh[pred_idx], prev_state, args)
    )
    safe_pred = (
        pred_idx != 0
...
def run_clip(model, disambiguator, criterion, data, epoch, args, cfg_obj):
    ...
    pred_correct_list = []
    target_nonzero_list = []
    baseline_center_error_list = []
    target_center_error_list = []
    high_iou_preserve_list = []
    center_better_list = []
    fallback_frames = 0
    ...
    for t, (image, gt_bbox_cpu) in enumerate(zip(search_images, search_bboxes)):
        ...
        try:
            ...
            topk_center_errors = compute_topk_center_errors(topk_bboxes_xywh, gt_bbox_cpu.to(device=device))
        ...
        baseline_center_error = topk_center_errors[0]

        # ------------------------------------------------------------------
        # Center-aware meaningful-switch target policy
        # ------------------------------------------------------------------
        # The previous policy used IoU-only best_idx for supervision after the
        # meaningful-switch gate. This improves robustness but can disturb
        # high-precision localization, especially when SUTrack top-1 is already
        # accurate. We therefore select the candidate by a center-aware quality
        # score and add a high-IoU preserve rule:
        #
        #   quality = IoU - center_lambda * normalized_center_error
        #
        # If top-1 already has high IoU, we keep top-1 unless the candidate has
        # a large IoU gain or a clearly better center location. This is designed
        # to protect SR0.75 while retaining real disambiguation gains.
        if bool(getattr(args, "use_center_aware_target", True)):
            topk_quality = topk_ious - float(args.center_lambda) * topk_center_errors
            _, quality_best_idx = torch.max(topk_quality, dim=-1)
            target_candidate_idx = quality_best_idx.long()
        else:
            target_candidate_idx = best_idx.long()

        target_candidate_iou = topk_ious[target_candidate_idx]
        target_candidate_center_error = topk_center_errors[target_candidate_idx]
        target_candidate_gain = target_candidate_iou - baseline_iou

        meaningful_switch = (
            bool(valid.item())
            and int(target_candidate_idx.item()) != 0
            and float(target_candidate_gain.item()) >= float(args.apply_gain_thr)
            and float(target_candidate_iou.item()) >= float(args.best_iou_thr)
        )

        high_iou_preserve = (
            bool(valid.item())
            and float(baseline_iou.item()) >= float(args.high_iou_preserve_thr)
            and float(target_candidate_gain.item()) < float(args.high_iou_preserve_gain)
        )

        center_better = (
            float(target_candidate_center_error.item()) + float(args.center_better_margin)
            < float(baseline_center_error.item())
        )

        if high_iou_preserve and not center_better:
            train_target_idx = torch.zeros_like(best_idx).long()
        elif meaningful_switch:
            train_target_idx = target_candidate_idx.long()
        else:
            train_target_idx = torch.zeros_like(best_idx).long()

        apply_label = bool(int(train_target_idx.item()) != 0 and bool(valid.item()))

        update_info = choose_update(
            logits_t=logits_t,
            topk_bboxes_xywh=topk_bboxes_xywh,
            topk_feats=topk_feats.squeeze(0),
            target_idx=train_target_idx.long(),
            valid=valid,
            epoch=epoch,
            args=args,
            prev_state=state,
        ...
        target_list.append(train_target_idx.long())
        ...
        pred_correct_list.append(float(update_info["pred_idx"] == int(train_target_idx.item()) and bool(valid.item())))
        target_nonzero_list.append(float(int(train_target_idx.item()) != 0 and bool(valid.item())))
        baseline_center_error_list.append(baseline_center_error.detach())
        target_center_error_list.append(target_candidate_center_error.detach())
        high_iou_preserve_list.append(float(high_iou_preserve))
        center_better_list.append(float(center_better))
    ...
    metrics = {
        ...
        "avg_baseline_center_error": float((torch.stack(baseline_center_error_list) * valid_float).sum().item() / denom),
        "avg_target_center_error": float((torch.stack(target_center_error_list) * valid_float).sum().item() / denom),
        "high_iou_preserve_ratio": sum(high_iou_preserve_list) / float(max(len(high_iou_preserve_list), 1)),
        "center_better_ratio": sum(center_better_list) / float(max(len(center_better_list), 1)),
...
def main():
    ...
    print(f"[INFO] apply_gain_thr         : {args.apply_gain_thr}")
    print(f"[INFO] best_iou_thr           : {args.best_iou_thr}")
    print(f"[INFO] use_center_aware_target: {args.use_center_aware_target}")
    print(f"[INFO] center_lambda          : {args.center_lambda}")
    print(f"[INFO] high_iou_preserve_thr  : {args.high_iou_preserve_thr}")
    print(f"[INFO] high_iou_preserve_gain : {args.high_iou_preserve_gain}")
    print(f"[INFO] center_better_margin   : {args.center_better_margin}")
    ...
    if args.resume:
        ...
        # 恢复优化器状态；如果参数组变化，跳过 optimizer 但不中断训练
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        totals = {
            ...
            "avg_best_iou": 0.0,
            "avg_iou_gain": 0.0,
            "avg_baseline_center_error": 0.0,
            "avg_target_center_error": 0.0,
            "high_iou_preserve_ratio": 0.0,
            "center_better_ratio": 0.0,
        ...
        for step, data in enumerate(pbar, start=1):
            ...
            for key in (
                ...
                "avg_best_iou",
                "avg_iou_gain",
                "avg_baseline_center_error",
                "avg_target_center_error",
                "high_iou_preserve_ratio",
                "center_better_ratio",
        ...
        metrics = {
            ...
            "avg_baseline_center_error": totals["avg_baseline_center_error"] / denom,
            "avg_target_center_error": totals["avg_target_center_error"] / denom,
            "high_iou_preserve_ratio": totals["high_iou_preserve_ratio"] / denom,
            "center_better_ratio": totals["center_better_ratio"] / denom,
        ...
        print(
            ...
            f"base_cerr={metrics['avg_baseline_center_error']:.4f} "
            f"target_cerr={metrics['avg_target_center_error']:.4f} "
            f"hiou_preserve_ratio={metrics['high_iou_preserve_ratio']:.4f} "
            f"center_better_ratio={metrics['center_better_ratio']:.4f} "
