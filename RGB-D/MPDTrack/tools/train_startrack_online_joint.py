...
def run_clip(model, disambiguator, criterion, data, epoch, args, cfg_obj):
    ...
    for t, (image, gt_bbox_cpu) in enumerate(zip(search_images, search_bboxes)):
        ...
        # ------------------------------------------------------------------
        # Meaningful-switch target policy
        # ------------------------------------------------------------------
        # Old behavior used best_idx as the training target whenever the frame
        # was valid. That makes the model learn many weak / noisy switches where
        # the oracle best candidate is only marginally better than top-1.
        #
        # New behavior only supervises a non-top1 switch when it is both valid
        # and meaningfully better. Otherwise, the target remains SUTrack top-1.
        # This keeps CE/Rank supervision and teacher-forced history consistent.
        apply_label = (
            bool(valid.item())
            and int(best_idx.item()) != 0
            and float(iou_gain.item()) >= float(args.apply_gain_thr)
            and float(best_iou.item()) >= float(args.best_iou_thr)
        )
        if apply_label:
            train_target_idx = best_idx.long()
        else:
