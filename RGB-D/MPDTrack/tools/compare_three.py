    ...
    try:
        ...
        for i in range(1, len(img_files)):
            ...
            peak_infos.append({
                ...
                "should_update_history": bool(peak_info.get("should_update_history", False)),
                "history_update_blocked": bool(peak_info.get("history_update_blocked", False)),
                "rerank_used_raw": bool(peak_info.get("rerank_used_raw", peak_info.get("rerank_used", False))),
                "use_refined_score": bool(peak_info.get("use_refined_score", False)),
                "safe_fallback_to_baseline": bool(peak_info.get("safe_fallback_to_baseline", False)),
                "takeover_allowed": bool(peak_info.get("takeover_allowed", False)),
                "takeover_block_reason": str(peak_info.get("takeover_block_reason", "")),
                "fallback_reason": str(peak_info.get("fallback_reason", "")),
                "takeover_center_jump_prev": float(peak_info.get("takeover_center_jump_prev", 0.0)),
                "takeover_center_jump_baseline": float(peak_info.get("takeover_center_jump_baseline", 0.0)),
                "takeover_width_ratio": float(peak_info.get("takeover_width_ratio", 1.0)),
                "takeover_height_ratio": float(peak_info.get("takeover_height_ratio", 1.0)),
                "takeover_area_ratio": float(peak_info.get("takeover_area_ratio", 1.0)),
                "takeover_cooldown_before": int(peak_info.get("takeover_cooldown_before", 0)),
                "takeover_cooldown_after": int(peak_info.get("takeover_cooldown_after", 0)),
                "takeover_pending_count_before": int(peak_info.get("takeover_pending_count_before", 0)),
                "takeover_pending_count_after": int(peak_info.get("takeover_pending_count_after", 0)),
...
def compute_switch_diagnostics(
    ...
    horizons=(5, 10, 30),
    iou_eps=0.01,
    change_iou_thresh=0.95,
):
    """Diagnose whether Stage4 switching / takeover is useful.

    This version explicitly separates:
      - raw rerank suggestion: rerank_used_raw
      - refined score usage: use_refined_score
      - actual state takeover: takeover_allowed / rerank_used
      - real history update: should_update_history
    """
    out_csv = Path(out_csv)
    ...
    s4_iou = bbox_iou_xywh(stage4_pred, gt)
    iou_gain = s4_iou - base_iou

    base_ce = center_error_xywh(base_pred, gt)
    s4_ce = center_error_xywh(stage4_pred, gt)
    ce_gain = base_ce - s4_ce

    pred_iou_between = bbox_iou_xywh(stage4_pred, base_pred)
    bbox_changed = pred_iou_between < change_iou_thresh

    peak_by_frame = {}
    for item in stage4_peak:
        try:
            peak_by_frame[int(item.get("frame", -1))] = item
        except Exception:
            continue
    ...
    for i in range(len(gt)):
        peak = peak_by_frame.get(i, {})

        rerank_used = _b(peak.get("rerank_used", False))
        rerank_used_raw = _b(peak.get("rerank_used_raw", rerank_used))
        use_refined_score = _b(peak.get("use_refined_score", False))
        safe_fallback_to_baseline = _b(peak.get("safe_fallback_to_baseline", False))
        takeover_allowed = _b(peak.get("takeover_allowed", rerank_used))

        should_update = _b(peak.get("should_update", False))
        should_update_history = _b(peak.get("should_update_history", False))
        history_update_blocked = _b(peak.get("history_update_blocked", False))

        selected_idx = _i(peak.get("selected_idx", -1))
        selected_idx_final = _i(peak.get("selected_idx_final", selected_idx))
        target_prob = _f(peak.get("target_prob", 0.0))
        identity_margin = _f(peak.get("identity_margin", 0.0))
        ambiguity_ratio = _f(peak.get("ambiguity_ratio", 0.0))
        history_len = _i(peak.get("history_len", -1))
        update_history_count = _i(peak.get("update_history_count", 0))

        improved = bool((iou_gain[i] > iou_eps) and valid[i])
        hurt = bool((iou_gain[i] < -iou_eps) and valid[i])
        same = bool((abs(iou_gain[i]) <= iou_eps) and valid[i])

        row = {
            "frame": int(i),
            "valid": int(valid[i]),
            "base_iou": float(base_iou[i]),
            "stage4_iou": float(s4_iou[i]),
            "iou_gain": float(iou_gain[i]),
            "base_ce": float(base_ce[i]),
            "stage4_ce": float(s4_ce[i]),
            "ce_gain": float(ce_gain[i]),
            "pred_iou_between": float(pred_iou_between[i]),
            "bbox_changed": int(bbox_changed[i]),

            "rerank_used": int(rerank_used),
            "rerank_used_raw": int(rerank_used_raw),
            "use_refined_score": int(use_refined_score),
            "safe_fallback_to_baseline": int(safe_fallback_to_baseline),
            "takeover_allowed": int(takeover_allowed),
            "takeover_block_reason": str(peak.get("takeover_block_reason", "")),
            "fallback_reason": str(peak.get("fallback_reason", "")),

            "should_update": int(should_update),
            "should_update_history": int(should_update_history),
            "history_update_blocked": int(history_update_blocked),

            "selected_idx": int(selected_idx),
            "selected_idx_final": int(selected_idx_final),
            "target_prob": float(target_prob),
            "identity_margin": float(identity_margin),
            "ambiguity_ratio": float(ambiguity_ratio),
            "history_len": int(history_len),
            "update_history_count": int(update_history_count),

            "takeover_center_jump_prev": _f(peak.get("takeover_center_jump_prev", 0.0)),
            "takeover_center_jump_baseline": _f(peak.get("takeover_center_jump_baseline", 0.0)),
            "takeover_width_ratio": _f(peak.get("takeover_width_ratio", 1.0)),
            "takeover_height_ratio": _f(peak.get("takeover_height_ratio", 1.0)),
            "takeover_area_ratio": _f(peak.get("takeover_area_ratio", 1.0)),
            "takeover_cooldown_before": _i(peak.get("takeover_cooldown_before", 0)),
            "takeover_cooldown_after": _i(peak.get("takeover_cooldown_after", 0)),
            "takeover_pending_count_before": _i(peak.get("takeover_pending_count_before", 0)),
            "takeover_pending_count_after": _i(peak.get("takeover_pending_count_after", 0)),
        ...
        for h in horizons:
            ...
            mask = valid[j0:j1]
            if mask.any():
                row[f"future_gain_{h}"] = float(iou_gain[j0:j1][mask].mean())
                row[f"future_hurt_frames_{h}"] = int(((iou_gain[j0:j1] < -iou_eps) & mask).sum())
                row[f"future_improve_frames_{h}"] = int(((iou_gain[j0:j1] > iou_eps) & mask).sum())
            else:
                row[f"future_gain_{h}"] = 0.0
                row[f"future_hurt_frames_{h}"] = 0
                row[f"future_improve_frames_{h}"] = 0
    ...
    def count(mask):
        return int((mask & valid_mask).sum())

    def first_frame(mask):
        idx = np.where(mask & valid_mask)[0]
        return int(idx[0]) if len(idx) > 0 else -1

    rerank_mask = np.array([bool(r["rerank_used"]) for r in rows], dtype=bool)
    raw_rerank_mask = np.array([bool(r["rerank_used_raw"]) for r in rows], dtype=bool)
    refined_mask = np.array([bool(r["use_refined_score"]) for r in rows], dtype=bool)
    takeover_mask = np.array([bool(r["takeover_allowed"]) for r in rows], dtype=bool)
    update_raw_mask = np.array([bool(r["should_update"]) for r in rows], dtype=bool)
    update_hist_mask = np.array([bool(r["should_update_history"]) for r in rows], dtype=bool)
    changed_mask = bbox_changed.astype(bool)

    improved_mask = (iou_gain > iou_eps)
    hurt_mask = (iou_gain < -iou_eps)
    same_mask = (np.abs(iou_gain) <= iou_eps)

    summary = {
        "valid_frames": int(valid_mask.sum()),
        "all_improved_frames": count(improved_mask),
        "all_hurt_frames": count(hurt_mask),
        "all_same_frames": count(same_mask),
        "all_mean_iou_gain": float(iou_gain[valid_mask].mean()) if valid_mask.any() else 0.0,

        "first_bbox_changed_frame": first_frame(changed_mask),
        "first_raw_rerank_frame": first_frame(raw_rerank_mask),
        "first_use_refined_score_frame": first_frame(refined_mask),
        "first_takeover_allowed_frame": first_frame(takeover_mask),
        "first_should_update_history_frame": first_frame(update_hist_mask),

        "raw_rerank_frames": count(raw_rerank_mask),
        "use_refined_score_frames": count(refined_mask),
        "takeover_allowed_frames": count(takeover_mask),

        "rerank_frames": count(rerank_mask),
        "rerank_improved_frames": count(rerank_mask & improved_mask),
        "rerank_hurt_frames": count(rerank_mask & hurt_mask),
        "rerank_same_frames": count(rerank_mask & same_mask),

        "takeover_improved_frames": count(takeover_mask & improved_mask),
        "takeover_hurt_frames": count(takeover_mask & hurt_mask),
        "takeover_same_frames": count(takeover_mask & same_mask),

        "bbox_changed_frames": count(changed_mask),
        "bbox_changed_improved_frames": count(changed_mask & improved_mask),
        "bbox_changed_hurt_frames": count(changed_mask & hurt_mask),
        "bbox_changed_same_frames": count(changed_mask & same_mask),

        "should_update_frames_raw": count(update_raw_mask),
        "should_update_history_frames": count(update_hist_mask),
        "should_update_history_improved_frames": count(update_hist_mask & improved_mask),
        "should_update_history_hurt_frames": count(update_hist_mask & hurt_mask),
    }

    summary["rerank_precision_gain"] = float(summary["rerank_improved_frames"] / max(summary["rerank_frames"], 1))
    summary["rerank_hurt_ratio"] = float(summary["rerank_hurt_frames"] / max(summary["rerank_frames"], 1))
    summary["takeover_precision_gain"] = float(summary["takeover_improved_frames"] / max(summary["takeover_allowed_frames"], 1))
    summary["takeover_hurt_ratio"] = float(summary["takeover_hurt_frames"] / max(summary["takeover_allowed_frames"], 1))
    summary["changed_precision_gain"] = float(summary["bbox_changed_improved_frames"] / max(summary["bbox_changed_frames"], 1))
    summary["changed_hurt_ratio"] = float(summary["bbox_changed_hurt_frames"] / max(summary["bbox_changed_frames"], 1))

    for h in horizons:
        future_vals = []
        future_vals_rerank = []
        future_vals_takeover = []
        future_vals_update = []
        for r in rows:
            if int(r["valid"]) <= 0:
                continue
            val = float(r[f"future_gain_{h}"])
            future_vals.append(val)
            if int(r["rerank_used"]) > 0:
                future_vals_rerank.append(val)
            if int(r["takeover_allowed"]) > 0:
                future_vals_takeover.append(val)
            if int(r["should_update_history"]) > 0:
                future_vals_update.append(val)
        summary[f"future_gain_{h}_all_mean"] = float(np.mean(future_vals)) if future_vals else 0.0
        summary[f"future_gain_{h}_rerank_mean"] = float(np.mean(future_vals_rerank)) if future_vals_rerank else 0.0
        summary[f"future_gain_{h}_takeover_mean"] = float(np.mean(future_vals_takeover)) if future_vals_takeover else 0.0
        summary[f"future_gain_{h}_update_history_mean"] = float(np.mean(future_vals_update)) if future_vals_update else 0.0

    with open(out_txt, "w", encoding="utf-8") as f:
        f.write("=" * 100 + "\n")
        f.write("Stage4 Switch / Safe-Takeover Diagnostics\n")
        f.write("=" * 100 + "\n\n")
        for k, v in summary.items():
            f.write(f"{k}: {v}\n")
        f.write("\n[Interpretation]\n")
        f.write("first_bbox_changed_frame vs first_takeover_allowed_frame tells whether drift starts after takeover.\n")
        f.write("use_refined_score_frames counts candidate refined-map usage before takeover gate.\n")
        f.write("takeover_allowed_frames counts actual state takeovers.\n")
        f.write("should_update_history_frames counts real memory updates after the outer safety gate.\n")
        f.write("future_gain_*_takeover_mean measures cascade effect after actual takeovers.\n")
...
def save_peak_info(path, peak_infos):
    ...
    with open(path, "w", encoding="utf-8") as f:
        fieldnames = [
            ...
            "history_update_blocked",
            "rerank_used_raw",
            "use_refined_score",
            "safe_fallback_to_baseline",
            "takeover_allowed",
            "takeover_block_reason",
            "fallback_reason",
            "takeover_center_jump_prev",
            "takeover_center_jump_baseline",
            "takeover_width_ratio",
            "takeover_height_ratio",
            "takeover_area_ratio",
            "takeover_cooldown_before",
            "takeover_cooldown_after",
            "takeover_pending_count_before",
            "takeover_pending_count_after",
            "selected_idx",
            "selected_idx_raw",
            "selected_idx_final",
            "target_prob",
...
def run_one_sequence(args, seq, offline_ckpt, stage4_ckpt):
    ...
    switch_diagnostics = compute_switch_diagnostics(
        base_pred=base_pred,
        stage4_pred=stage4_pred,
        gt=gt,
        valid=valid,
        stage4_peak=stage4_peak,
        out_csv=out_dir / "stage4_switch_diagnostics.csv",
        out_txt=out_dir / "stage4_switch_diagnostics.txt",
    )
    ...
    summary = {
        ...
        "files": {
            ...
            "stage4_switch_diagnostics_csv": str(out_dir / "stage4_switch_diagnostics.csv"),
            "stage4_switch_diagnostics_txt": str(out_dir / "stage4_switch_diagnostics.txt"),
        },
    }
    ...
    print("[Switch diagnostics]")
    print(json.dumps(switch_diagnostics, indent=2))
...
def run_one_sequence(args, seq, offline_ckpt, stage4_ckpt):
    ...
    # Extra diagnostic outputs for dense/SOI sequences.
    # This does not change any CLI arguments or inference behavior.
    switch_diagnostics = compute_switch_diagnostics(
        base_pred=base_pred,
        stage4_pred=stage4_pred,
        gt=gt,
...
def main():
    ...
    parser.add_argument("--baseline-param", type=str, default="sutrack_b384")
    ...
    parser.add_argument("--star-mode", type=str, default="safe_predicted", help="Optional override: coach/gated/shadow/etc.")

    parser.add_argument("--out-dir", type=str, default="threeway_eval5.16_v5")
    parser.add_argument(
