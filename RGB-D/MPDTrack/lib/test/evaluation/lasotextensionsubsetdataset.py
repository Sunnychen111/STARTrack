    ...
    def _construct_sequence(self, sequence_name):
        ...
        if os.path.exists(nlp_path):
            try:
                ...
                # 防止 numpy array 被转成奇怪格式
                if isinstance(nlp_rect, np.ndarray):
                    nlp_rect = " ".join(nlp_rect.reshape(-1).tolist())
                else:
                    nlp_rect = str(nlp_rect)

            except Exception as e:
                print(f"[WARN] failed to load nlp.txt for {sequence_name}: {e}")
                nlp_rect = ""
        else:
            print(f"[WARN] nlp.txt not found for {sequence_name}, use empty language.")
            nlp_rect = ""
