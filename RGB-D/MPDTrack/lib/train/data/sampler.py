...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def _get_max_gap_value(self):
        if isinstance(self.max_gap, (list, tuple)):
            return max(self.max_gap)
        return int(self.max_gap)

    def _get_consecutive_end_ids(self, visible, num_ids=1, min_id=None, max_id=None):
        if num_ids <= 0:
            return []

        if min_id is None or min_id < (num_ids - 1):
            min_id = num_ids - 1
        if max_id is None or max_id > len(visible):
            max_id = len(visible)

        valid_end_ids = []
        for end_id in range(min_id, max_id):
            start_id = end_id - num_ids + 1
            if start_id < 0:
                continue
            if all(bool(visible[idx]) for idx in range(start_id, end_id + 1)):
                valid_end_ids.append(end_id)
        return valid_end_ids

    def _select_template_history_ids(self, history_ids, num_ids):
        if num_ids == 0:
            return []
        if len(history_ids) < num_ids:
            return None
        if num_ids == 1:
            return [history_ids[0]]

        select_indices = np.linspace(0, len(history_ids) - 1, num=num_ids, dtype=int).tolist()
        template_frame_ids = [history_ids[idx] for idx in select_indices]
        if len(set(template_frame_ids)) != num_ids:
            return None
        return template_frame_ids
        ...
        while not valid:
            try:
                ...
                if is_video_dataset:
                    ...
                    elif self.frame_sample_mode == "online_temporal":
                        template_frame_ids, search_frame_ids = self.get_frame_ids_online_temporal(visible)
                ...
                seq_name = None
                if hasattr(dataset, "sequence_list") and seq_id < len(dataset.sequence_list):
                    seq_name = dataset.sequence_list[seq_id]
                data['seq_name'] = seq_name if seq_name is not None else str(seq_id)
                data['seq_id'] = seq_id
    ...
    def get_frame_ids_online_temporal(self, visible):
        # sample a temporally ordered training clip that matches online testing more closely:
        # search frames are consecutive and chronological, while templates come from earlier visible history.
        gap_increase = 0
        max_gap = self._get_max_gap_value()

        while True:
            valid_end_ids = self._get_consecutive_end_ids(
                visible,
                num_ids=self.num_search_frames,
                min_id=self.num_search_frames - 1,
                max_id=len(visible),
            )
            if len(valid_end_ids) == 0:
                return None, None

            candidate_end_ids = []
            for end_id in valid_end_ids:
                search_start_id = end_id - self.num_search_frames + 1
                history_min_id = max(0, search_start_id - max_gap - gap_increase)
                history_ids = [idx for idx in range(history_min_id, search_start_id) if bool(visible[idx])]
                if len(history_ids) >= self.num_template_frames:
                    candidate_end_ids.append(end_id)

            if len(candidate_end_ids) == 0:
                gap_increase += 5
                if gap_increase > 1000:
                    print("too large image gap, check the sampler, current gap: " + str(gap_increase))
                continue

            search_end_id = random.choice(candidate_end_ids)
            search_start_id = search_end_id - self.num_search_frames + 1
            search_frame_ids = list(range(search_start_id, search_end_id + 1))

            history_min_id = max(0, search_start_id - max_gap - gap_increase)
            history_ids = [idx for idx in range(history_min_id, search_start_id) if bool(visible[idx])]
            template_frame_ids = self._select_template_history_ids(history_ids, self.num_template_frames)
