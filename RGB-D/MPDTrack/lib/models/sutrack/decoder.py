...
class CenterPredictor(nn.Module, ):
    ...
    def forward(self, x, gt_score_map=None):
        """ Forward pass with input x. """
        score_map_ctr, size_map, offset_map, f_map = self.get_score_map(x) # x: torch.Size([b, c, h, w])
        ...
        return score_map_ctr, bbox, size_map, offset_map, f_map
    ...
    def cal_bbox_at_peaks(self, peak_xy, size_map, offset_map):
        """Decode CENTER-head boxes at supplied feature-map peaks.

        This is the same size/offset formula used by cal_bbox(), extended from
        the argmax top-1 index to externally supplied top-k peak coordinates.
        """
        if peak_xy.dim() != 3 or peak_xy.size(-1) != 2:
            raise ValueError(f"peak_xy must be [B, K, 2], got {tuple(peak_xy.shape)}")
        bsz, num_peaks, _ = peak_xy.shape
        idx_x = peak_xy[..., 0].long().clamp(0, self.feat_sz - 1)
        idx_y = peak_xy[..., 1].long().clamp(0, self.feat_sz - 1)
        flat_idx = (idx_y * self.feat_sz + idx_x).view(bsz, 1, num_peaks).expand(bsz, 2, num_peaks)

        size = size_map.flatten(2).gather(dim=2, index=flat_idx).transpose(1, 2)
        offset = offset_map.flatten(2).gather(dim=2, index=flat_idx).transpose(1, 2)

        bbox = torch.cat([
            (idx_x.to(torch.float).unsqueeze(-1) + offset[..., :1]) / self.feat_sz,
            (idx_y.to(torch.float).unsqueeze(-1) + offset[..., 1:]) / self.feat_sz,
            size,
        ], dim=-1)
        return bbox
    ...
    def get_score_map(self, x):
        ...
        f_map = x_ctr4
        score_map_ctr = self.conv5_ctr(f_map)
        ...
        return _sigmoid(score_map_ctr), _sigmoid(score_map_size), score_map_offset, f_map
