...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,
        ...
        # For full_sequence mode we precompute sliding-window indices across all sequences
        if self.frame_sample_mode == 'full_sequence':
            self.full_windows = []  # list of (dataset_index, seq_id, start_frame)
            win_len = self.num_template_frames + self.num_search_frames
            # Sparse stride: cover a wider temporal range per window (e.g. every 3 frames)
            self._full_seq_stride = 3
            effective_span = win_len * self._full_seq_stride
            try:
                for di, ds in enumerate(self.datasets):
                    num_seq = ds.get_num_sequences()
                    for seq_id in range(num_seq):
                        seq_info = ds.get_sequence_info(seq_id)
                        visible = seq_info['visible']
                        L = len(visible)
                        if L < effective_span:
                            # Fall back to dense window if sequence is too short for sparse
                            if L >= win_len:
                                for start in range(0, L - win_len + 1):
                                    self.full_windows.append((di, seq_id, start, 1))
                            continue
                        for start in range(0, L - effective_span + 1):
                            self.full_windows.append((di, seq_id, start, self._full_seq_stride))
            except Exception:
                # If any dataset doesn't support the fast listing APIs, fall back to normal random sampling
                self.full_windows = []

            # Shuffle windows to avoid strict ordering across epochs (will still be deterministic within this process)
            if len(getattr(self, 'full_windows', [])) > 0:
                import random as _random
                _random.shuffle(self.full_windows)
            self._full_index = 0
            self._full_epoch_count = 0
    ...
    def __getitem__(self, index):
        if self.train_cls:
            ...
            if self.frame_sample_mode == 'full_sequence' and getattr(self, 'full_windows', None):
                return self.getitem_full_sequence()
            return self.getitem()

    def getitem_full_sequence(self):
        """
        Sparse sliding-window traversal across full sequences. Each window uses a temporal
        stride so that N frames cover a wider time range (e.g. stride=3: frames [0,3,6,9,12]).
        Windows are reshuffled whenever we wrap around the full list (once per epoch).
        """
        if not getattr(self, 'full_windows', None):
            return self.getitem()

        total = len(self.full_windows)
        attempts = 0
        while attempts < min(total, 200):
            idx = self._full_index % total
            # Reshuffle when wrapping around (new epoch of windows)
            if self._full_index > 0 and idx == 0:
                self._full_epoch_count += 1
                import random as _random
                _random.shuffle(self.full_windows)
            self._full_index += 1
            attempts += 1

            di, seq_id, start, stride = self.full_windows[idx]
            dataset = self.datasets[di]
            win_len = self.num_template_frames + self.num_search_frames

            all_frame_ids = [start + i * stride for i in range(win_len)]
            template_frame_ids = all_frame_ids[:self.num_template_frames]
            search_frame_ids = all_frame_ids[self.num_template_frames:]

            try:
                seq_info = dataset.get_sequence_info(seq_id)
                template_frames, template_anno, meta_obj_train = dataset.get_frames(seq_id, template_frame_ids, seq_info)
        ...
        # Fall back to random sampling if sequential windows keep failing.
        return self.getitem()
    ...
    def getitem(self):
        ...
        while not valid:
            try:
                ...
                try:
                    if 'template_frames' in locals() and len(template_frames) > 0:
                        print("template frame shape:", template_frames[0].shape)
                    if 'search_frames' in locals() and len(search_frames) > 0:
                        print("search frame shape:", search_frames[0].shape)
                    if 'dataset' in locals():
                        print("dataset:", dataset.get_name())
                except Exception:
                    pass
                valid = False
