...
def sample_target(im, target_bb, search_area_factor, output_sz=None, mask=None):
    ...
    if not all(math.isfinite(v) for v in [x, y, w, h]):
        raise Exception(f'Invalid bounding box with non-finite values: {[x, y, w, h]}')
    if w <= 0 or h <= 0:
        raise Exception(f'Invalid bounding box size: w={w}, h={h}')
    ...
    if im_crop.size == 0 or im_crop.shape[0] == 0 or im_crop.shape[1] == 0:
        raise Exception(
            'Crop is empty after jitter. '
            f'BBox={[x, y, w, h]}, crop_sz={crop_sz}, image_hw={im.shape[:2]}, '
            f'crop_xy=({x1},{y1})-({x2},{y2})'
        )

    # Pad
    im_crop_padded = cv.copyMakeBorder(im_crop, y1_pad, y2_pad, x1_pad, x2_pad, cv.BORDER_CONSTANT)
    if im_crop_padded.size == 0 or im_crop_padded.shape[0] == 0 or im_crop_padded.shape[1] == 0:
        raise Exception(
            'Padded crop is empty. '
            f'BBox={[x, y, w, h]}, crop_sz={crop_sz}, image_hw={im.shape[:2]}, '
            f'pad=({x1_pad},{x2_pad},{y1_pad},{y2_pad})'
        )
