...
def hdf5_depth_loader(path):
    """专门用来读取 .h5 格式的半精度深度图，并还原为网络需要的 float32 格式。"""
    try:
        # 使用 with 语句现开现关，防止多进程 Dataloader 死锁
        with h5py.File(path, 'r') as f:
            # 读取 float16 并转回 float32 给 PyTorch
            depth_float32 = f['depth'][:].astype(np.float32)
            
        # 增加通道维度，使其形状变为 [H, W, 1]
        depth_out = np.expand_dims(depth_float32, axis=-1)
        return depth_out
    except Exception as e:
        print('ERROR: Could not read HDF5 depth map "{}"'.format(path))
        print(e)
        return None