...
class BaseTrainer:
    ...
    def _load_torch_checkpoint(self, checkpoint_path):
        """Load checkpoint with explicit weights_only behavior across torch versions."""
        try:
    ...
    def load_checkpoint(self, checkpoint = None, fields = None, ignore_fields = None, load_constructor = False):
        ...
        # Load network
        checkpoint_dict = self._load_torch_checkpoint(checkpoint_path)
    ...
    def load_state_dict(self, checkpoint=None, distill=False):
        ...
        checkpoint_state = checkpoint_dict["net"]
        ...
        for key, value in checkpoint_state.items():
            ...
            if key == "encoder.body.patch_embed.proj.weight" and value.ndim == 4 and target_value.ndim == 4:
                out_ok = value.shape[0] == target_value.shape[0]
                spatial_ok = value.shape[2:] == target_value.shape[2:]
                if out_ok and spatial_ok and value.shape[1] >= target_value.shape[1]:
                    filtered_state[key] = value[:, :target_value.shape[1], :, :]
                    print(f"Adapted {key} from {tuple(value.shape)} to {tuple(filtered_state[key].shape)}")
                    continue
        ...
        missing_k, unexpected_k = net.load_state_dict(filtered_state, strict=False)
