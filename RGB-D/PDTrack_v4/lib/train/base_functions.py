...
def update_settings(settings, cfg):
    ...
    settings.lasot_split = getattr(cfg.DATA.TRAIN, "LASOT_SPLIT", "train")
    settings.lasot_split_file = getattr(cfg.DATA.TRAIN, "LASOT_SPLIT_FILE", "")
    seq_filter_cfg = getattr(cfg.DATA, "SEQ_FILTER", None)
    settings.seq_filter_enable = getattr(seq_filter_cfg, "ENABLE", False) if seq_filter_cfg is not None else False
    settings.lasot_sequence_filter_path = getattr(seq_filter_cfg, "LASOT_PATH", "") if seq_filter_cfg is not None else ""
...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        assert name in ["LASOT", "GOT10K_vottrain", "GOT10K_votval", "GOT10K_train_full", "GOT10K_hard",
        ...
        if name == "LASOT":
            if settings.use_lmdb:
                ...
                datasets.append(Lasot_lmdb(settings.env.lasot_lmdb_dir, split=settings.lasot_split, image_loader=image_loader,
                                           ...
                                           split_file=settings.lasot_split_file,
                                           sequence_filter_file=settings.lasot_sequence_filter_path if settings.seq_filter_enable else None))
            else:
                datasets.append(Lasot(settings.env.lasot_dir, split=settings.lasot_split, image_loader=image_loader,
                                      multi_modal_vision=settings.multi_modal_vision,
                                      multi_modal_language=settings.multi_modal_language,
                                      use_nlp=settings.use_nlp['LASOT'],
                                      split_file=settings.lasot_split_file,
        ...
        if name == "GOT10K_hard":
            if settings.use_lmdb:
                print("Building got10k_hard from lmdb")
                datasets.append(Got10k_lmdb(settings.env.got10k_lmdb_dir, split='hard', image_loader=image_loader,
                                            multi_modal_vision=settings.multi_modal_vision,
                                            multi_modal_language=settings.multi_modal_language,
                                            use_nlp=settings.use_nlp['GOT10K']
                                            ))
            else:
                datasets.append(Got10k(settings.env.got10k_dir, split='hard', image_loader=image_loader,
                                       multi_modal_vision=settings.multi_modal_vision,
                                       multi_modal_language=settings.multi_modal_language,
                                       use_nlp=settings.use_nlp['GOT10K']
                                       ))
