...
if __name__ == '__main__':
    ...
    total_time = 0  # 用于计算平均 FPS
    
    for k, filename in enumerate(filenames):
        raw_image = cv2.imread(filename)
        
        # 2. 开始计时
        # 如果是 GPU 推理，torch.cuda.synchronize() 可以确保计时更准确（虽然 infer_image 返回 numpy 往往已隐含同步）
        if DEVICE == 'cuda':
        ...
        # 4. 打印到终端 (文件名 | 耗时 | FPS)
        print(f'Progress {k+1}/{len(filenames)}: {os.path.basename(filename)} | Time: {inference_time*1000:.1f}ms | FPS: {fps:.2f}')
    ...
    # 5. 最后输出平均 FPS
    if len(filenames) > 0:
        avg_fps = len(filenames) / total_time
        print("-" * 50)
