...
class iTPN(nn.Module):
    ...
    def forward_stage1_2(self, template_list, search_list, template_anno_list):
        """
        执行 Patch Embedding 和 Stage 1 & 2 (下采样阶段)
        返回: 拼接好的 [B, N_search+N_template, C] 格式的 Tokens
        """
        # 1. 运行原有的 prepare_tokens_with_masks (包含 PatchEmbed 和 Mask 处理)
    ...
    def forward_stage3(self, xz, rpe_index=None):
        """
        执行 Stage 3 (主要 Transformer Block) 和 最终 Norm
        输入: 融合后的特征 xz
        """
        # 运行剩下的 Block (即 Stage 3)
        # self.blocks[:-self.num_main_blocks] 是 Stage 1&2 (在 prepare_tokens_with_masks 里跑过了)
        # self.blocks[-self.num_main_blocks:] 是 Stage 3
        for blk in self.blocks[-self.num_main_blocks:]:
