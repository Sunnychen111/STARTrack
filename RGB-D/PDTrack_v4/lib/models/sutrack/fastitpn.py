...
class Fast_iTPN(nn.Module):
    ...
    # === [Modification for Mamba Fusion] ===
    # 将 forward_features 拆分为两个阶段，以便在中间插入 Mamba 融合
    
    def forward_stage1_2(self, template_list, search_list, template_anno_list, text_src=None, task_index=None):
        """
        FastITPN 第一阶段: 执行 Patch Embedding + Stage 1 & 2
        """
        # 调用现有的 prepare_tokens_with_masks 函数
        # 注意：必须提供 defaults (text_src=None)，因为 Encoder 可能不会传这些参数
        xz = self.prepare_tokens_with_masks(template_list, search_list, template_anno_list, text_src, task_index)
        
        # Positional Dropout
        xz = self.pos_drop(xz)

        # 获取相对位置编码 (FastITPN 使用的是 rel_pos_bias 而不是 index)
        rel_pos_bias = self.rel_pos_bias() if self.rel_pos_bias is not None else None
            
        # 返回特征和位置编码 (对应 Encoder 里的 rpe_index)
        return xz, rel_pos_bias

    def forward_stage3(self, xz, rel_pos_bias=None):
        """
        FastITPN 第二阶段: 执行 Stage 3 (Transformer Main Blocks) + Norm
        """
        # 运行剩下的 Block (Stage 3)
        for blk in self.blocks[-self.num_main_blocks:]:
            # 支持梯度检查点 (Checkpointing) 以节省显存
            if self.grad_ckpt:
                xz = checkpoint.checkpoint(blk, xz, rel_pos_bias)
            else:
...
def load_pretrained(model, checkpoint, pos_type, patchembed_init):
    ...
    # === 修改开始：智能处理 Patch Embedding 权重 ===
    patch_embedding_weight = model.state_dict()['patch_embed.proj.weight'] # 当前模型的权重形状
    patch_embedding_weight_pretrained = state_dict['patch_embed.proj.weight'] # 预训练权重的形状 (通常是 6通道)
    
    # 获取当前模型的输入通道数
    current_in_chans = patch_embedding_weight.shape[1] # 应该是 3
    pretrained_in_chans = patch_embedding_weight_pretrained.shape[1] # 应该是 6 (SUTrack预训练)

    print(f"Loading weights: Pretrained({pretrained_in_chans}ch) -> Model({current_in_chans}ch)")

    if current_in_chans == 3:
        # 【情况 1】: Mono-RGBDL 模式 (我们现在的模式)
        # 只加载预训练权重的前 3 个通道 (RGB)，丢弃后面的通道
        if pretrained_in_chans >= 3:
            patch_embedding_weight = patch_embedding_weight_pretrained[:, :3, :, :]
        else:
            raise ValueError("Pretrained weight has less than 3 channels!")
            
    elif current_in_chans == 6:
        # 【情况 2】: 原版 SUTrack RGB-T 模式
        if patchembed_init == "copy":
            # 复制 RGB 到后三个通道
            patch_embedding_weight[:,:3,:,:] = patch_embedding_weight_pretrained[:,:3,:,:] 
            # 注意：预训练权重本身可能已经是6通道，或者是3通道需要复制
            if pretrained_in_chans == 3:
                patch_embedding_weight[:,3:,:,:] = patch_embedding_weight_pretrained
            else:
                patch_embedding_weight[:,3:,:,:] = patch_embedding_weight_pretrained[:,3:,:,:]
...
@register_model
def fastitpnb(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy", in_chans=6, **kwargs):
    # [修改点] 接收 in_chans 参数 (默认6)，并传给 Fast_iTPN
    model = Fast_iTPN(
        patch_size=16, in_chans=in_chans, embed_dim=512, depth_stage1=3, depth_stage2=3, depth=24, num_heads=8, bridge_mlp_ratio=3.,
        mlp_ratio=3., qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6),
        convmlp=True,
        naiveswiglu=True,
        subln=True,
...
@register_model
def fastitpnl(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy", in_chans=6, **kwargs):
    # [修改点] 接收 in_chans 参数 (默认6)，并传给 Fast_iTPN
    model = Fast_iTPN(
        patch_size=16, in_chans=in_chans, embed_dim=768, depth_stage1=2, depth_stage2=2, depth=40, num_heads=12, bridge_mlp_ratio=3.,
        mlp_ratio=3., qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6),
        convmlp=True,
        naiveswiglu=True,
        subln=True,
